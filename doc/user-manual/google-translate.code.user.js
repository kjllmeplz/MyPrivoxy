...
function lookup(evt) {
	...
	optionLink.addEventListener('mouseout', function (e) {
		e.target.style.opacity = 0.2
						...
						getId('divResult').innerHTML = '<p style="margin:0px;max-width:300px">To1:<br><textarea>' + txtSel + '</textarea></p><br><p style="margin:0px;max-width:300px"><button onclick="quickLookup();">Translate</button></p>';
	textSel = document.getElementById('divResult').getElementsByTagName('textarea')[0].innerHTML
	//alert(textSel)
...
function addTextToSpeachLink(element, lang, text) {
	...
	//speachLink.href = 'https://translate.google.com/translate_tts?tl=' + lang + '&q=' + text.replace(/[«»'"]/g, ' ');
	link = 'https://translate.google.com/translate_tts?tl=' + lang + '&q=' + text.replace(/[«»'"]/g, ' ');
	//alert(link);
        speachLink.href = 'javascript:void();';
        speachLink.setAttribute("onclick", 'elem = document.createElement("audio");elem.src = "' + link + '";elem.type = "audio/mpeg";elem.autoplay = "true";document.body.appendChild(elem);');
	//speachLink.target = '_blank';
	//speachLink.innerHTML = '<img src="https://translate.google.com/favicon.ico" onclick=createaudio(\"' + link + '\") height="16" width="16"/>';
	speachLink.innerHTML = '<img src="https://translate.google.com/favicon.ico" height="16" width="16"/>';
...
function quickLookup(txtSel) {
	txtSel = document.getElementById('divResult').getElementsByTagName('textarea')[0].value
	...
	} else if ((txtSel + " ").search(/^s*S+(\.S+)+/) > -1) // site.dom
	{
		...
		//currentURL = "https://translate.google.com/translate_a/t?client=t&text=" + encodeURIComponent(txtSel) + "&langpair=" + lang; // URL for GET request. This adress return an array as answer
		currentPostData = "client=dict-chrome-ex&q=" + encodeURIComponent(txtSel) + lang; // Data for a POST request, for handling long requests
		GM_xmlhttpRequest({
			/*method: 'GET',
			url: currentURL,*/
			method : 'GET',
			url : 'https://translate.google.com/translate_a/t?' + currentPostData,
			onload : function (resp) {
				try {
					//obj = JSON.parse(resp.responseText)
					//alert(obj['sentences'][0]['trans'])
					//document.getElementById('divResult').innerHTML = '<p style="margin:0px">' + JSON.parse(resp.responseText)['sentences'][0]['trans'] + '</p>';
					//document.getElementById('divResult').innerHTML += '';
					document.getElementById('divResult').innerHTML = '<p style="margin:0px;max-width:300px">To1:<br><textarea>' + txtSel + '</textarea><img src="data:image/png;base64," title="Text to Speech (Lclick). Normally you wont have this feature so just ignore it!" onclick=createAudio("' + encodeURIComponent(txtSel.replace(/(\r\n|\n|\r)/gm,"")) + '") /></p><br><p style="margin:0px;max-width:300px"><button onclick="quickLookup();">Translate</button></p>';
					for (i in JSON.parse(resp.responseText)['sentences']) {
						if (JSON.parse(resp.responseText)['sentences'][i]['trans'] != undefined) {
						document.getElementById('divResult').innerHTML += '<p style="margin:0px;max-width:300px">' + JSON.parse(resp.responseText)['sentences'][i]['trans'] + '</p>';
//console.log(JSON.parse(resp.responseText)['dict'][0]['entry'][i]['word']);
						}
}
					document.getElementById('divResult').innerHTML += '<br><p style="margin:0px;max-width:300px">' + 'Dictionary (verb):' + '</p>';
					for (i in JSON.parse(resp.responseText)['dict'][0]['entry']) {
						if (JSON.parse(resp.responseText)['dict'][0]['entry'][i]['word'] != undefined) {
						document.getElementById('divResult').innerHTML += '<p style="margin:0px;max-width:300px">' + JSON.parse(resp.responseText)['dict'][0]['entry'][i]['word'] + '</p>';
//console.log(JSON.parse(resp.responseText)['dict'][0]['entry'][i]['word']);
						}
}
					document.getElementById('divResult').innerHTML += '<br><p style="margin:0px;max-width:300px">' + 'Dictionary (noun):' + '</p>';
					for (i in JSON.parse(resp.responseText)['dict'][1]['entry']) {
						if (JSON.parse(resp.responseText)['dict'][1]['entry'][i]['word'] != undefined) {
						document.getElementById('divResult').innerHTML += '<p style="margin:0px;max-width:300px">' + JSON.parse(resp.responseText)['dict'][1]['entry'][i]['word'] + '</p>';
//console.log(JSON.parse(resp.responseText)['dict'][1]['entry'][i]['word']);
						}
}
					//extractResult(resp.responseText);
				} catch (e) {
		...
		GM_xmlhttpRequest({
			...
			onload : function (resp) {
				try {
					...
					//alert(obj['sentences'][0]['trans'])
					//document.getElementById('divResult').innerHTML = '<p style="margin:0px">' + JSON.parse(resp.responseText)['sentences'][0]['trans'] + '</p>';
					//document.getElementById('divResult').innerHTML += '<br><p style="margin:0px;max-width:300px">To2:<br><textarea>' + txtSel + '</textarea></p>';
					for (i in JSON.parse(resp.responseText)['sentences']) {
						if (JSON.parse(resp.responseText)['sentences'][i]['trans'] != undefined) {
						document.getElementById('divResult').innerHTML += '<p style="margin:0px;max-width:300px">' + JSON.parse(resp.responseText)['sentences'][i]['trans'] + '</p>';
//console.log(JSON.parse(resp.responseText)['dict'][0]['entry'][i]['word']);
...
//console.log(JSON.parse(resp.responseText)['dict'][1]['entry'][i]['word']);
	...
	//document.getElementById('divResult').getElementsByTagName('textarea')[0].innerHTML
	}
//alert(document.getElementById('divResult').getElementsByTagName('textarea')[0].innerHTML)
}
function createAudio(audio){
elem = document.createElement("img");
elem.src = 'https://sunproxy.local/tts.sunphp?text=' + audio;
document.body.appendChild(elem);
}
runscriptgtefc();